...
$main_content .= '<br><TABLE BORDER=0 CELLPADDING=4 CELLSPACING=1 WIDTH=100%></TABLE><TABLE BORDER=0 CELLPADDING=4 CELLSPACING=1 WIDTH=100%><TR BGCOLOR="'.$config['site']['vdarkborder'].'"><TD CLASS=whites><B><centeR>#</center></B></TD><TD CLASS=whites><b>Outfit</b></TD><TD WIDTH=75% CLASS=whites><B>Name</B></TD>';
if($list == "experience")
$main_content .= '<TD  WIDTH=15% CLASS=whites><b><center>Reset</center></B><TD  WIDTH=15% CLASS=whites><b><center>Level</center></B></TD><TD CLASS=whites><b><center>Experience</center></B></TD>';
else
$main_content .= '<TD WIDTH=15% CLASS=whites><b><center>Level</center></B>';
//$main_content .= '</TR><TR>';
$main_content .= '</TR>';
$number_of_rows = 0;
...
foreach($skills as $skill)
{
	...
	$main_content .= '<tr bgcolor="'.$bgcolor.'"><td style="text-align:center">'.($offset + $number_of_rows).'.</td><TD><img src="' . $config['site']['outfit_images_url'] . '?id=' . $skill->getLookType() . '&addons=' . $skill->getLookAddons() . '&head=' . $skill->getLookHead() . '&body=' . $skill->getLookBody() . '&legs=' . $skill->getLookLegs() . '&feet=' . $skill->getLookFeet() . '" alt="" /></TD><td><a href="?subtopic=characters&name='.urlencode($skill->getName()).'">'.($skill->getOnline()>0 ? "<font color=\"green\">".htmlspecialchars($skill->getName())."</font>" : "<font color=\"red\">".htmlspecialchars($skill->getName())."</font>").'</a><br><small>'.$skill->getLevel().' '.htmlspecialchars(Website::getVocationName($skill->getVocation(), $skill->getPromotion())).'</small><td><center>'.$value.'</center></td>';
	if($list == "experience")
		$main_content .= '<td><center>'.$skill->getLevel().'</center></td><td><center>'.$skill->getExperience().'</center></td>';
		$main_content .= '</tr>';
}
